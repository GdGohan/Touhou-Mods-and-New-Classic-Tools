...
def convert_folder(input_dir, output_dir, recursive=True):
    """Converte todos os arquivos .opus de uma pasta para .ogg.

    Se recursive=True, subpastas também são pesquisadas e recriadas
    ...
    if not input_dir.is_dir():
        raise NotADirectoryError(f"pasta de entrada não existe: {input_dir}")

    pattern = "**/*.opus" if recursive else "*.opus"
    files = sorted(input_dir.glob(pattern))

    if not files:
        print(f"Nenhum arquivo .opus encontrado em: {input_dir}")
        return 0
    ...
    for in_path in files:
        relative = in_path.relative_to(input_dir)
        out_path = output_dir / relative.with_suffix(".ogg")
        out_path.parent.mkdir(parents=True, exist_ok=True)

        try:
            n, granule = convert(str(in_path), str(out_path))
            print(
                f"[OK] {in_path} -> {out_path} "
                f"({n} packets, {granule} samples, {granule / 48000:.2f}s)"
            )
            converted += 1
        except Exception as ex:
            print(f"[ERRO] {in_path}: {ex}", file=sys.stderr)
            failed += 1

    print()
    print(f"Concluído: {converted} convertido(s), {failed} erro(s).")
...
def main():
    convert_folder(
        "TouhouNewClassic/bgm",
        "TouhouNewClassic/bgm_out"
    )
